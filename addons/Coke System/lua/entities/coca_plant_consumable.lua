...
ENT.PrintName = "Coca Plant (Consumable)"
...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "coca_plant_consumable" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	return ent
...
function ENT:FullyGrown()
	return self:GetNWInt( "Growth" ) == 600
end
...
function ENT:Think()
	...
	if IsValid( tr.Entity ) and tr.Entity:GetClass() == "heat_lamp" and SERVER then
		if tr.Entity:GetNWInt( "TurnedOn" ) then
			self:SetNWInt( "Growth", math.Clamp( growth + 1, 0, 600 ) )
		end
	end
...
if CLIENT then
    ...
    function ENT:Draw()
		...
		if self:GetPos():DistToSqr( plyShootPos ) < 562500 then
			...
			local title2 = "Progress: "..CalcPercentage( growth, 600 ).."%"
			...
			cam.Start3D2D(pos,ang,0.1)
				...
				if self:GetNWInt( "Growth" ) == 600 then
